...
export const Timeline: FC<{ data: TimelineProps[] }> = ({ data }) => {
  // const ype = "paid";
  return (
    <ul>
      {data.map((item, index) => (
                  ...
                  {item.level.stage === 1 && (
                    <div className="flex justify-between">
                      <p className="text-sm font-semibold text-gray-800">
                        {item.author.bioData.fullName}{" "}
                        <span className="text-gray-500 text-xs font-normal">
                          {item.action} a request
                        </span>
                      </p>
                      <time
                        dateTime={"activityItem.dateTime"}
                        className="flex py-0.5 leading-5 text-xs font-light items-center "
                      >
                        <span>
                          <svg
