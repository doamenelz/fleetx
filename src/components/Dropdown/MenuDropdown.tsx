              ...
              <MenuItem key={index}>
                {item.type === "link" ? (
                  <Link
                    ...
                    className="text-sm pl-3 py-2.5 items-center text-left gap-2 w-full hover:bg-slate-900 flex justify-between"
                    key={item.id}
                  >
                    {item.label}
                  </Link>
  ...
  return (
    ...
    >
      ...
      <MenuItems
        transition
        anchor="bottom end"
        className=" mt-2 w-56 z-20 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 transition focus:outline-none data-[closed]:scale-95 data-[closed]:transform data-[closed]:opacity-0 data-[enter]:duration-100 data-[leave]:duration-75 data-[enter]:ease-out data-[leave]:ease-in"
      >
        <div className="p-2">
